...
<div id="main">
    <div id="toolbar">
        ...
        <a class="waves-effect waves-button" href="javascript:;" onclick="updateAction(undefined)"><i class="zmdi zmdi-edit"></i>&nbsp;更改状态</a>
        <a class="waves-effect waves-button" href="javascript:;" onclick="deleteAction(undefined)"><i class="zmdi zmdi-close"></i>&nbsp;删除</a>
    </div>
...
<script>
    ...
    $(function () {
        ...
        $table.bootstrapTable({
            url: '${manageBaseUrl}/dinnerTables',
            method: 'GET',
//            height: getHeight(),
            striped: true,
//            search: true,
            ...
            columns: [
                ...
                {field: 'id', title: '编号', sortable: true, halign: 'center'},
                {
                    ...
                    <#list dinnerTableStatusList as dinnerTableStatus>
                        if (${dinnerTableStatus.key}== value
                        )
                        {
                            return "${dinnerTableStatus.value!}"
                        }
                    </#list>
                    }
                },
        ...
        $.confirm({
            ...
            buttons: {
                confirm: {
                    ...
                    action: function () {
                        request('${manageBaseUrl}/dinnerTables', requestType.POST, dataType.JSON, undefined, function (result) {
    ...
    function updateAction(id) {
        ...
        if (!id && rows.length == 0) {
            ...
            $.confirm({
                ...
                animationSpeed: 300,
                title: false,
                content: '是否修改餐桌状态？'/*$('#updateDialog').html()*/,
                buttons: {
                    confirm: {
                        text: '确认',
                        btnClass: 'waves-effect waves-button',
                        action: function () {
                            ...
                            request('${commonBaseUrl}/batchOperation', requestType.POST, dataType.JSON, {
                                ...
                                if (result.code != undefined && result.code != 0) {
                                    ...
                                    request('${manageBaseUrl}/dinnerTables/' + result.data, requestType.PUT, dataType.JSON, undefined, function (result) {
                                        if (result.code != undefined && result.code != 0) {
                                            $.alert(result.message);
                                        } else {
                                            $table.bootstrapTable('refresh');
                        ...
                        action: function () {
                            ...
                            request('${commonBaseUrl}/batchOperation', requestType.POST, dataType.JSON, {
                                ...
                                if (result.code != undefined && result.code != 0) {
                                    ...
                                    request('${manageBaseUrl}/dinnerTables/' + result.data, requestType.DELETE, dataType.JSON, undefined, function (result) {
                                        if (result.code != undefined && result.code != 0) {
                                            $.alert(result.message);
                                        } else {
                                            $table.bootstrapTable('refresh');
