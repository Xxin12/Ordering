...
<script>
    var baseUrl = '${manageBaseUrl}/foods?time='+new Date().getTime();
...
//            console.log(value, row, index);
    ...
    function createAction() {
        $.confirm({
            ...
            animationSpeed: 300,
            title: '新增',
            content: "url:${manageBaseUrl}/food_detail.html",
            buttons: {
                confirm: {
                    text: '确认',
                    btnClass: 'waves-effect waves-button',
                    action: function () {
                        request("${manageBaseUrl}/foods", requestType.POST,dataType.JSON,$("form").serializeArray(),function(result){
    ...
    function updateAction(id) {
        $.confirm({
            ...
            height:300,
            title: '编辑',
            content: "url:${manageBaseUrl}/food_detail.html?id="+id,
            buttons: {
                confirm: {
                    text: '确认',
                    btnClass: 'waves-effect waves-button',
                    action: function () {
                        request("${manageBaseUrl}/foods/" + id, requestType.PUT,dataType.JSON,$("form").serializeArray(),function(result){
                            if (result.code != undefined && result.code != 0) {
                                $.alert(result.message);
                            } else {
                                $table.bootstrapTable('refresh');
                        ...
                        action: function () {
                            ...
                            request('${commonBaseUrl}/batchOperation', requestType.POST, dataType.JSON, {
                                ...
                                if (result.code != undefined && result.code != 0) {
                                    ...
                                    request('${manageBaseUrl}/foods/' + result.data, requestType.DELETE, dataType.JSON, undefined, function (result) {
                                        if (result.code != undefined && result.code != 0) {
                                            $.alert(result.message);
                                        } else {
                                            $table.bootstrapTable('refresh');
     ...
     * 选择食品分类
     */
    $("#search [name='foodTypeId']").bind("change", function () {
        $table.bootstrapTable('refresh', {
            url: baseUrl + "&foodTypeId=" + $(this).val()
        });
    });
</script>
</body>
