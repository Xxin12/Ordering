...
<script>
    ...
    $(function () {
        ...
        $table.bootstrapTable({
            url: '${manageBaseUrl}/temporaryOrders?finished=true',
            method: 'GET',
//            height: getHeight(),
            striped: true,
//            search: true,
            ...
            columns: [
                ...
                {field: 'foodName', title: '食品名称', sortable: true, halign: 'center'},
                {field: 'orderId', title: '所属订单', sortable: true, halign: 'center'},
                {field: 'number', title: '数量', sortable: true, halign: 'center'},
                {field: 'totalPrice', title: '总价', sortable: true, halign: 'center' , formatter: function (value, row, index) {return value / 100}},
                {field: 'finishedTime', title: '完成时间', sortable: true, halign: 'center'},
                {
                    title: '操作',
                    halign: 'center',
                    align: 'center',
                        ...
                        action: function () {
                            ...
                            request('${commonBaseUrl}/batchOperation', requestType.POST, dataType.JSON, {
                                ...
                                if (result.code != undefined && result.code != 0) {
                                    ...
                                    request('${manageBaseUrl}/temporaryOrders/' + result.data, requestType.DELETE, dataType.JSON, undefined, function (result) {
                                        if (result.code != undefined && result.code != 0) {
                                            $.alert(result.message);
                                        } else {
