...
//            console.log(value, row, index);
    ...
    function createAction() {
        $.confirm({
            ...
            animationSpeed: 300,
            title: '新增',
            content: "url:${manageBaseUrl}/food_type_detail.html",
            buttons: {
                confirm: {
                    text: '确认',
                    btnClass: 'waves-effect waves-button',
                    action: function () {
                        request("${manageBaseUrl}/foodTypes", requestType.POST,dataType.JSON,$("form").serializeArray(),function(result){
    ...
    function updateAction(id) {
        $.confirm({
            ...
            animationSpeed: 300,
            title: '编辑',
            content: "url:${manageBaseUrl}/food_type_detail.html?id="+id,
            buttons: {
                confirm: {
                    text: '确认',
                    btnClass: 'waves-effect waves-button',
                    action: function () {
                        request("${manageBaseUrl}/foodTypes/" + id, requestType.PUT,dataType.JSON,$("form").serializeArray(),function(result){
                            if (result.code != undefined && result.code != 0) {
                                $.alert(result.message);
                            } else {
                                $table.bootstrapTable('refresh');
                        ...
                        action: function () {
                            ...
                            request('${commonBaseUrl}/batchOperation', requestType.POST, dataType.JSON, {
                                ...
                                if (result.code != undefined && result.code != 0) {
                                    ...
                                    request('${manageBaseUrl}/foodTypes/' + result.data, requestType.DELETE, dataType.JSON, undefined, function (result) {
                                        if (result.code != undefined && result.code != 0) {
                                            $.alert(result.message);
                                        } else {
                                            $table.bootstrapTable('refresh');
